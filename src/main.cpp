...
void handleLineLogic(std::vector<std::string>& tokens, std::vector<std::string>& history, bool needToWait = true) {
    ...
    if(tokens[0] == "exit") {
        ...
        if(tokens.size() > 2 && tokens[1] == "-r") {
            ...
            // IMPORTANT: history is a reference in this function, but the sync index is global. 
            // Correctly updating it here.
...
int main() {
    ...
    // FIX: Check if env var exists before using it
    char* histfile_env = std::getenv("HISTFILE");
    if (histfile_env) {
        std::ifstream hist(histfile_env);
        if (hist.is_open()) {
            std::string s;
            while(getline(hist, s)) history.push_back(s);
        }
        // FIX: Update sync index so we don't duplicate old commands on write
        history_file_sync_index = history.size();
    }

    int history_index = history.size(); // Start at end of history
    char c;
